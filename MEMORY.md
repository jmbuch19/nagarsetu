...
- **Governance label: "Nagarsetu Admins" everywhere user-facing.** Both Editor and Admin roles are collapsed into the single user-facing label "Nagarsetu Admins" — used in UI copy, disclaimers, magazine credits, notifications, etc. The schema keeps the `members.role (member|editor|admin)` split because magazine and moderation flows need it technically, but the public never sees that distinction. **Specific powers per role + first humans assigned to each are deferred until launch is in sight** (added to AGENDA §8 Hardening). Gujarati equivalent of the label TBD at i18n time.
