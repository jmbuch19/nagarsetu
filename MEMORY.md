...
- **Profile fields:** required = full name, surname, city, **PIN code**, gender, DOB. Encouraged/skippable = email (OPTIONAL — weekly digest/reminders/receipts), photo, sub-community, bio. Then smart profiling (profession→specialty→status→opt-ins). Fully usable with just a verified phone.
...
- **Unlisted cities — suggest-and-approve.** When a member's city isn't in the seeded list, they tap "Suggest this city" → inserts a `cities` row with `status = 'pending'`. The member uses it immediately (zero blocking friction). Admin reviews periodically and either approves (`status → 'approved'`), rejects, or **merges** into a canonical city (`status → 'merged'` + `merged_into = canonical_id`). Application read paths follow `merged_into` so picking "NYC" still aggregates to "New York". Preserves the controlled-list discipline (no free-text city) without ever blocking a diaspora signup. Schema in `supabase/migrations/20260526160000_lookups_schema.sql` (cities.status + cities.merged_into).
...
- **Smart progressive profiling:** branching + consent-first. Profession → specialty (cascading, auto-appears) → **years_experience (optional)** → **expertise_text (optional, "what you actually did", ~280 chars, on `member_professions`)** → status (current/retired/studying) → opt-in expert guidance → opt-in other offers (mentor/other). Retired expertise still counts; studying = mentee candidate. **Two complementary fields:** `member_professions.expertise_text` is "what I do" (always-on profile fact); `member_capabilities.description` is "what I'll help with" (consent-gated service offer) — don't conflate. Capabilities stored in `member_capabilities` (non-paid સેવા offers), editable anytime, with later contextual nudges. System matches contribution to need — never ranks people.
