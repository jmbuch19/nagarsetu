...
- **FAQ** (`FAQ.md` → `/faq` page): comprehensive, **community-voiced** ("a fellow Nagar from Ahmedabad", no marketing voice), covering every feature we've designed — joining, profile, connecting, money (no commission, only listing fee), offering (Earning/Security/Safety/Service), PG seeker+provider, magazine, verified help drives, WhatsApp/digest discipline, privacy/DPDP, deceased/memorialisation, events/scholarships, platform promises/limits. Gujarati-first typography; link from footer + near landing-page Sign in. Open-ended to grow with new features.
