...
- **Session & device policy (Phase 1).** Members stay effectively logged in for **30 days per device** on **unlimited devices**. Mechanism = Supabase's standard **short access token (1 h)** + **rolling 30-day refresh token**; the SDK silently refreshes in the background, so the member sees no OTP prompt unless they go 30 days without opening the app on that device. **Never make the access JWT itself long-lived** — that breaks revocation. Refresh tokens stored in platform-secure storage (httpOnly cookies on web; expo-secure-store / iOS Keychain / Android Keystore on mobile — never AsyncStorage or localStorage). **Active sessions UI is mandatory before launch** (list devices + per-device sign-out + "sign out everywhere" via admin revocation RPC). Driver: cut Meta WhatsApp OTP delivery cost (~₹0.30–₹0.50 per send; ~4× annual saving for active members) without weakening revocation. See `SPEC.md` §1.5.
